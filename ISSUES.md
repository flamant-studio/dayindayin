...
3. Sebastian chose plain white everywhere (not the more radical all-white-page variant, which would've dropped the site's warm chalk identity site-wide) + a border to delimit the panel. Final: 2px solid `var(--c-text)`. PLP border wraps image+title+price, stops before the "View product" CTA (his explicit instruction). PDP border wraps the main image only (background was already `#fff` for contain mode, no change needed there).
**Status:** 🟡 CLAIMED (`acee87a`) · Verified against a genuinely fresh build — mid-session caught that a `git checkout` revert had left the dev server on a stale `.next` build still containing the discarded parchment experiment, rebuilt before this verification. Live-verified against production: `a[href^="/shop/"]` computed border is `2px solid rgb(26,23,20)`. Screenshots: `screenshots/final-*.png` (desktop+mobile) and `screenshots/live-final-plp2.png` (production).
