...
**Verified:** `npx next build` clean; ran `next start` locally and confirmed both PDPs' rendered `<head>` now show `og:image`/`twitter:image` pointing at `https://dayindayin-site.vercel.app/.../opengraph-image` at 1200×630, plus `og:site_name`. Pushed — will re-verify against the live URL + opengraph.xyz once Vercel deploys.
**Status:** ✅ done — 2 real bugs fixed (oversized/wrong-aspect og:image, missing site_name), 1 pre-existing sitewide issue found and flagged but correctly left alone (homepage og:image unreachable until DNS points).
