...
4. **Correction (`f449cf4`):** `.page` background on `/shop` and `/shop/[handle]` now set to `var(--c-white)` — the actual page canvas is white, not just the image box. Nav and Footer (shared global chrome) intentionally left on chalk — not part of this discussion, would need a separate call. PLP border moved from `.cardInner` to `.card` — wraps image+title+price+CTA as one bordered unit, per his corrected instruction.
**Status:** 🟡 CLAIMED (`f449cf4`) · Live-verified against production this time (not just localhost): `getComputedStyle` on the PDP `.page` returns `rgb(255,255,255)`; on the PLP `.card` returns `2px solid rgb(26,23,20)`. Pixel-sampled the screenshot (not just eyeballed) — page margins and card gaps read `(255,255,255)`. Screenshots: `screenshots/v2-*.png` (local) and `screenshots/live-final2-*.png` (production).
