...
**Follow-up, same day (Sebastian asked directly: are signups tagged? can they opt out?):**
- Tags: were not being set at all (`tags: []` confirmed on the test customers) — no way to segment "signed up via newsletter" from a real paying customer. Added a `newsletter-signup` tag on `customerCreate`.
- Opt-out: the `emailMarketingConsent` field is Shopify's real, correct consent state — any compliant sender (Shopify Email, or Klaviyo/Mailchimp connected the official way) will read it, add an unsubscribe link automatically, and flip it to `UNSUBSCRIBED` when clicked. Nothing is actually sending campaigns yet, so there's no live unsubscribe experience today — but the data model supports it correctly once sending starts.
- **Real bug caught in the same pass, before shipping:** the existing-customer path (someone who already has a Shopify account, e.g. a past buyer, signing up for the newsletter) was using a raw email-interpolated search string (`customers(query: "email:${email}")`) to look the customer up — a public endpoint taking arbitrary user input straight into a query DSL, a real injection surface. Switched to `customerByIdentifier(identifier: {emailAddress})`, a typed scalar lookup, before this ever went live.
- **Second real bug, caught via live testing before declaring done:** tagging existing customers used `tagsAdd` (correct, additive) but consent used `customerUpdate(input: {emailMarketingConsent})` — Shopify rejects this outright on existing customers ("please use the customerEmailMarketingConsentUpdate Mutation instead"), a completely separate dedicated mutation from the one `customerCreate` accepts inline. Found via a real failed live test (`502`), not assumed correct from documentation. Fixed and re-verified.
- **Live-verified, both paths, against Shopify directly:** a brand-new email and a re-signup by an already-existing customer both now show `tags: ["newsletter-signup"]` and `emailMarketingConsent.marketingState: SUBSCRIBED`.
**Status:** 🟡 CLAIMED — signup genuinely works now, verified against Shopify directly, not just the API's own claim of success. Tagging and existing-customer opt-in confirmed working for both new and existing customers.
...
### Pre-launch task 2: Fine Art PDP image audit — all 98 works checked, 14 fixed, 6 need a decision
**Task:** for every fine-art work, is the first image in the PDP's "Studio views" gallery (`gallery[0]`) a genuine full shot of the complete piece — separate from the hero image at the top of the page, which is allowed to be a close-up/detail crop by design?
**Method:** parallelized across 8 research agents — every work got at least one image actually downloaded and viewed (not judged by filename); non-compliant works got their full gallery + hero checked for an existing full shot, then Stine's entire Dropbox archive searched for an unused one.
**Result — 79 of 98 already correct.** 19 needed action:
- **7 fixed by reorder** (full shot already existed on the PDP, just wasn't first): `birds` (hero promoted into gallery), `bedroom-wall-rug`, `gud-har-meldt-afbud`, `elsk`, `mariann`, `blue-branch`, `universe-collection`.
- **6 fixed with a new image from Dropbox**, each confirmed by viewing the candidate before use: `candy-v`, `fleur-de-lys` (this one had no gallery array at all before), `red-on-wood`, `person-walking`, `taped-objects`, `purple-fabric-in-garden`. The last two required converting the original RAW `.nef` capture via `sips` — no processed version existed anywhere. Uploaded to the live Vercel Blob store, verified 200 on every URL before wiring into `lib/data.ts`.
- **6 have no full shot anywhere, checked exhaustively (site + full Dropbox archive)** — genuinely nothing to fix without new photography: `du-und`, `cat-doll` (every file is a sketch or a close-up of the piece mid-process), `ingenting` (the one wide frame that exists has the text mirrored/backwards — the same defect that got its old hero image rejected before), `colour-study-blue` (only one image was ever taken, an extreme crop), `linen-and-yellow-flower` (none of the 5 files on record match the described scene at all), `polaroids-on-fabrics` (the fabric backdrop runs off-frame in every shot found). **Status: needs Sebastian's decision** — reshoot vs. accept as-is.
**Follow-up bug found and fixed along the way:** Sebastian confirmed `bedroom-rug` and `bedroom-wall-rug` (which the audit flagged as sharing byte-identical photos) are the same physical piece, not a mix-up — merged into one `Work` entry (kept `bedroom-wall-rug`), added a permanent redirect from the old URL. That redirect silently didn't fire on the first deploy: `app/works/[slug]/page.tsx` had no `revalidate` window, so the removed slug kept serving its old statically-cached page indefinitely instead of hitting the redirect. Fixed with `export const revalidate = 300` (matches the existing shop PDP pattern) — confirmed the redirect works live afterward.
**Also surfaced, not fixed (outside this task's strict scope, flagged for Sebastian rather than silently touched):** `rainbow-i`'s hero + all 4 gallery slots are the exact same file repeated; `hej`'s gallery images 2+ show a visibly different colorway than gallery[0] (likely a folder mix-up); `candy-v`'s description says "ring/doughnut" but the confirmed photo shows a hook/cane shape; `birds`' description says "three bird silhouettes" but the actual canvas is an abstract multi-bird composition; a handful of "OK" verdicts (`on-the-light-table`, `colourful-shadows`, `apple-scraps`, `green-flower`) were close calls on abstract/tightly-composed subjects, technically passing but worth a human glance.
**Status:** ✅ done for the 14 fixable works, deployed and verified live · ⏸ BLOCKED on the 6 with no source image (needs Sebastian: reshoot or accept) · full per-work verdict list exists as a standalone report, not committed to the repo.
