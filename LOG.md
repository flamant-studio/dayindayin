...
## 2026-08-29 — Store state note (Cowork)
dayindayin.dk is on Shopify **Pause and Build** since 2026-08-17 — the site is live but **checkout is disabled** (no orders can complete). Logged here because no dayindayin-site doc recorded it (portfolio-audit drift #1). Open question for Sebastian: is this a deliberate park, or is it meant to be re-upgraded to a checkout-enabled plan?

## 2026-08-30 — Fine Art: Tufting Works copy & metadata pass (Stine's 2026-07-30 recording)
Implemented Stine's confirmed edits to `lib/data.ts` for the tufting archive (renames, medium/category corrections, series tags, global "cut pile"/"cotton loop pile"/"hand-tufted"-for-Sitspots cleanup). 19 slug renames, each with a permanent redirect added to `next.config.ts` (old → new, chained where a slug had already been redirected once before, e.g. bedroom-rug → bedroom-wall-rug → you-and-i-and-the-universe now goes straight to the final slug). Added a new `handhooking` category (Purple Sun) wired into all three places category labels are hardcoded (`app/works/[slug]/page.tsx`, `app/fine-art/page.tsx`, `app/about/page.tsx`). Added three new optional `Work` fields not previously in the schema: `series` (cross-work grouping tag — Candy, Bifrost, Liebes Panopticon, Floral Things), `companionSlug` (Orange Sun ↔ Green Earth, rendered as a "Companion piece" link on both PDPs), `availabilityNote` (overrides the Sold/Original label text — used for Purple Sun's "Not for sale — currently with Ulrikke").

**Judgment calls made, not explicitly in the brief — flagging for review:**
- Jewel Hand's leftover description sentence said "wool" after removing the two flagged sentences; changed it to "cotton" to match the corrected Medium field (was going to ship as a visible contradiction otherwise).
- Tufted Mask and Candy Cane's existing copy read as AI-flourish/didn't match the corrected technique, so replaced in full with the brief's fallback copy rather than partial edits.
- Round Earth (not in the brief's work list) still says "Cut pile, high density." — left untouched per "works not listed are unchanged," flagged in a code comment. Global rule 1 technically conflicts with this; deferred to the explicit unlisted-works instruction.
- Floral Thing's existing copy had no forbidden terms, judged factual/clean, kept as-is per the conditional instruction.

Verified: `npx tsc --noEmit` clean, `next build` succeeded (97 static work pages), and spot-checked redirects + rendered pages on a local production server (wallflower, star, candy-9, jewel-hand, purple-sun's availability note, sitspot has zero "hand-tufted" occurrences, orange-sun ↔ green-earth companion links, handhooking category filter). Not yet pushed — Sebastian to review the judgment calls above before this goes live and breaks/redirects real URLs.
