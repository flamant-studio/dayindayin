...
| Gelato store / catalog | 🔄 **(2026-07-23)** Root cause of "broken variant selectors" found and fixed for Fine Art Print/Poster/Framed Print/Art Print: most products only had the CSV-import default size (and, for Framed Print, only the default frame colour) live even though the template supports more. Full size/colour grid now added and verified across all 78 products in these categories. Separately found and fixed a real Shopify data bug affecting the same products: a redundant, no-image, generically-priced "ghost" variant set sitting alongside every real one (379 deleted storewide, pre-existing, not caused by this fix). Mugs, tanks, totes, apparel, postcards, etc. not yet re-audited this way. **`GELATO_STRATEGY.md`'s curate-vs-automate decision is still open** — this fix closes one specific root cause, not the whole catalog-correctness question. **(2026-08-02)** Deeper, separate bug found: the original June bulk-creation only ever connects the *first* variant per product (confirmed via Gelato's own API — externalId/SKU linking mechanism now fully mapped, see LOG.md). Fixed 12 tank tops + 12 mugs via a confirmed repeatable recipe (delete phantom variants, fix SKU, collapse Shopify options to Default Title, republish); 2 mugs need Sebastian's call on missing color variants; 7 products stuck in a new `publishing_queued` state, unresolved, escalation to Gelato support offered but not yet sent (Sebastian's call). Also found this same bug was silently misassigning 52 products to the fallback shipping profile — see Shipping row below, fixed as a side effect of the shipping redesign, not the sync bug itself. |
...
| Newsletter API | ✅ **(2026-08-03)** Fixed, live-verified against Shopify directly. Signups save as Shopify customers with `emailMarketingConsent.marketingState: SUBSCRIBED` (not Mailchimp/Klaviyo — piggybacks on Shopify's own customer list, by original design). Three real bugs fixed: token was never actually minted (`SHOPIFY_ADMIN_TOKEN` doesn't exist as a static var in this project), the mutation used a Shopify field removed from the current API schema, and production's `SHOPIFY_STORE_DOMAIN`/`SHOPIFY_CLIENT_ID`/`SHOPIFY_CLIENT_SECRET` were corrupted/stale vs. the working local values. Full detail: `ISSUES.md` LB-3. |
