...
| Newsletter API | ✅ **(2026-08-03)** Fixed, live-verified against Shopify directly. Signups save as Shopify customers tagged `newsletter-signup`, `emailMarketingConsent.marketingState: SUBSCRIBED` (not Mailchimp/Klaviyo — piggybacks on Shopify's own customer list, by original design). Existing customers (past buyers) signing up get tagged + opted in via a separate `customerEmailMarketingConsentUpdate` path rather than silently skipped. No unsubscribe link exists yet — that's automatic once a real send tool (Shopify Email or a connected ESP) is wired up, not before. Full detail: `ISSUES.md` LB-3. |
